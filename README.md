...
- faster member access on bound classes: the current `bindMethod` / `bindProperty` dispatch routes every `obj.x` / `obj:m()` access through a Lua C function (`propertyIndexDispatcher`) that walks `__methods` → `__getters` on each lookup. For types bound with methods only (no properties), a future optimisation can set `__index` to a plain methods table — Lua's VM resolves that in a single rawget without crossing the C boundary, which is several times cheaper in hot loops. Worth measuring before generalising.